...
static HRESULT FillInitData(_In_ size_t width,
	...
	_Out_writes_to_ptr_(mipCount*arraySize) D3D12_SUBRESOURCE_DATA** initData)
{
	if (!bitData || !initData)
	{
		return E_POINTER;
	...
	_Out_writes_to_ptr_(mipCount*arraySize) D3D12_SUBRESOURCE_DATA** initData)
{
	...
	else
	{
		...
		// Create the texture
		*initData = new (std::nothrow) D3D12_SUBRESOURCE_DATA[mipCount * arraySize];
		if (!initData)
		{
			return E_OUTOFMEMORY;
		}
	...
	DDS_ALPHA_MODE* alphaMode)
{
	return CreateDDSTextureFromFileEx(d3dDevice, nullptr, fileName, maxsize,
		false, textureView, textureDesc, arraySize, initData, alphaMode);
}
	...
	DDS_ALPHA_MODE* alphaMode)
{
	...
	hr = CreateTextureFromDDS(d3dDevice, d3dContext, header,
		bitData, bitSize, maxsize, forceSRGB, textureView, textureDesc, arraySize, initData);
