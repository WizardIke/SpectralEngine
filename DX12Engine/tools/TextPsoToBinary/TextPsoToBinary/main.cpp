...
static GraphicsPipelineStateDesc readFromTextFile(std::ifstream& inFile, bool& succeeded)
{
	...
	while (getline(inFile, line))
	{
		...
		{
			psoDesc.vertexShader = data;
		}
		else if (name == "PixelShader")
		{
			psoDesc.pixelShader = data;
		}
...
static void writeToCppHeaderFile(const GraphicsPipelineStateDesc& psoDesc, std::ofstream& outFile, std::string_view className)
{
	outFile << "#pragma once\n";
...
int main(int argc, char** argv)
{
	...
	bool succeeded;
	GraphicsPipelineStateDesc psoDesc = readFromTextFile(infile, succeeded);
	infile.close();
	if (!succeeded)
	{
		return 1;
	...
	std::string outFileName = replaceFileExtension(inFileName, ".h");
	std::ofstream outFile(outFileName);
	writeToCppHeaderFile(psoDesc, outFile, ((std::string_view)outFileName).substr(0u, outFileName.size() - 5));
	outFile.close();
