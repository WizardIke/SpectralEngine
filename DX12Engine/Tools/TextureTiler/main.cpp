...
namespace
{
	...
	DdsHeaderDx12 textureInfoToDdsHeaderDx12(const TextureInfo& textureInfo)
	{
	...
	void untile(const TextureInfo& rextureInfo, char* sourcedata, char* destData)
	{
		uint32_t tileWidth, tileHeight, tileWidthBytes;
		tileWidthAndHeightAndTileWidthInBytes(rextureInfo.format, tileWidth, tileHeight, tileWidthBytes);

		for (std::size_t currentMipLevel = 0u; currentMipLevel != rextureInfo.mipLevels; ++currentMipLevel)
		{
			...
			std::size_t slicePitch, rowPitch, numRows;
			surfaceInfo(subresouceWidth, subresourceHeight, rextureInfo.format, slicePitch, rowPitch, numRows);

			const std::size_t numColumns = rowPitch / tileWidthBytes;
			const std::size_t partialColumnBytes = rowPitch % tileWidthBytes;
			const std::size_t subresourceSize = slicePitch * subresourceDepth;

			auto* currentUploadBuffer = destData;
			const auto* currentDepthSourceStart = sourcedata;
			for (std::size_t currentDepth = 0u; currentDepth != subresourceDepth; ++currentDepth)
			{
				std::size_t rowsDown = 0u;
				std::size_t remainingRows = numRows;
				while (remainingRows != 0u)
				{
					std::size_t tileHeightOnDisk = tileHeight;

					if (remainingRows < tileHeight) tileHeightOnDisk = remainingRows;
					const std::size_t tileSizeOnDisk = tileHeightOnDisk * tileWidthBytes;

					for (std::size_t row = 0u; row != tileHeightOnDisk; ++row)
					{
						for (std::size_t column = 0u; column != numColumns; ++column)
						{
							//copy row of current column
							const auto* currentSourceTileRowStart = currentDepthSourceStart + rowsDown * rowPitch + tileSizeOnDisk * column + tileWidthBytes * row;
							const auto* const currentSourceTileRowEnd = currentSourceTileRowStart + tileWidthBytes;
						...
						const auto* currentSourceTileRowStart = currentDepthSourceStart + rowsDown * rowPitch + tileSizeOnDisk * numColumns + partialColumnBytes * +row;
						const auto* const currentSourceTileRowEnd = currentSourceTileRowStart + partialColumnBytes;
						for (; currentSourceTileRowStart != currentSourceTileRowEnd;)
						{
							*reinterpret_cast<uint32_t*>(currentUploadBuffer) = *reinterpret_cast<const uint32_t*>(currentSourceTileRowStart);
							currentUploadBuffer += sizeof(uint32_t);
	...
	void tile(const TextureInfo& rextureInfo, char* sourcedata, char* destData)
	{
		uint32_t tileWidth, tileHeight, tileWidthBytes;
		tileWidthAndHeightAndTileWidthInBytes(rextureInfo.format, tileWidth, tileHeight, tileWidthBytes);
		...
		for (std::size_t currentMipLevel = 0u; currentMipLevel != rextureInfo.mipLevels; ++currentMipLevel)
		{
			...
			const auto* currentDestBuffer = sourcedata;
			auto* currentDepthSourceStart = destData;
			for (std::size_t currentDepth = 0u; currentDepth != subresourceDepth; ++currentDepth)
			{
				std::size_t rowsDown = 0u;
				std::size_t remainingRows = numRows;
				while (remainingRows != 0u)
				{
					...
					for (std::size_t row = 0u; row != tileHeightOnDisk; ++row)
					{
						...
						{
							//copy row of current column
							auto* currentSourceTileRowStart = currentDepthSourceStart + rowsDown * rowPitch + tileSizeOnDisk * column + tileWidthBytes * row;
							auto* const currentSourceTileRowEnd = currentSourceTileRowStart + tileWidthBytes;
							for (; currentSourceTileRowStart != currentSourceTileRowEnd;)
							{
						...
						//copy part of row of current column, length partialColumnBytes
						auto* currentSourceTileRowStart = currentDepthSourceStart + rowsDown * rowPitch + tileSizeOnDisk * numColumns + partialColumnBytes * +row;
						auto* const currentSourceTileRowEnd = currentSourceTileRowStart + partialColumnBytes;
						for (; currentSourceTileRowStart != currentSourceTileRowEnd;)
						{
							*reinterpret_cast<uint32_t*>(currentSourceTileRowStart) = *reinterpret_cast<const uint32_t*>(currentDestBuffer);
							currentDestBuffer += sizeof(uint32_t);
...
int main(int argc, char** argv)
{
	...
	TextureInfo textureInfo = getDDSTextureInfoFromFile(file);
	...
	outFile.write(convertedData.get(), dataLength);
